...
function loadData() {
    d3.csv("data/halo_contributions_projects.csv", function (a) {
        dataSource.house = a, data = a, d3.csv("data/halo_contributions_senate.csv", function (a) {
            dataSource.senate = a, initialize()
        })
    })
} function initialize() {
    ...
    viz.data(data).width(800).height(600).haloKey(function (a) {
        ...
        return a.PROJ_ID
    }).nodeGroupKey(function (a) {
        return a.CPNY
    }).value(function (a) {
        return Number(a.EXPERIENCE_GAIN)
    }).on("update", onUpdate)
        .on("nodeover", node_onMouseOver)
        .on("nodeout", onMouseOut)
        .on("arcover", arc_onMouseOver)
        .on("arcout", onMouseOut)
    ...
    */
    ...
    var d = {},
        e = viz.selection().selectAll(".vz-halo-link-path.node-key_" + b.key);
        e.each(function (a) {
            var b = viz.selection().selectAll(".vz-halo-arc.halo-key_" + viz.haloKey()(a.data));
            b.each(function (a) { d[a.data.key] || (d[a.data.key] = 1, createPacLabel(a.x, a.y, a.data.values[0].LANGUAGE)) })
        });
        var g = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect(),
    ...
    var d = viz.selection().selectAll(".vz-halo-link-path.halo-key_" + b.data.key),
        ...
        e = "$" + d3.format(",.0f")(e);
        var f = d3.selectAll(".vz-halo-node-plot")[0][0].getBoundingClientRect();
        createDataTip(b.x + f.left + f.width / 2, b.y + f.top + 100, "Projects done in", b.data.values[0].LANGUAGE/*, "Total Donated: " + e*/)
} function link_onMouseOver(a, b, c) {
    var d = viz.selection().selectAll(".vz-halo-node.node-key_" + viz.nodeKey()(b.data)),
    e = d.datum(),
    f = "$" + d3.format(",.0f")(viz.value()(b.data)),
    g = b.data.CPNY/*b.data.Month + "/" + b.data.Day + "/" + b.data.Year*/,
    h = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect();
    createDataTip(e.x + h.left + e.r, e.y + e.r + 25 + h.top, g, /*e.data.values[0].PROJ_NAME,*/ b.data.PROJ_NAME, "Received: " + f);

    var i = viz.selection().selectAll(".vz-halo-arc.halo-key_" + viz.haloKey()(b.data));
    e = i.datum(),
    createPacLabel(e.x, e.y, e.data.values[0].LANGUAGE)
} function createDataTip(a, b, c, d, e) {
    var f = datatip.replace("HEADER1", c);
    if (d.split(',')[1]) { d1 = "<li class='li-box'>" + d.split(',')[1] + "</li>" } else { d1 = "" };
    if (d.split(',')[2]) { d2 = "<li class='li-box'>" + d.split(',')[2] + "</li>" } else { d2 = "" };
    f = f.replace("HEADER2", "<li class='li-box'>" + d.split(',')[0] + "</li>" + d1 + d2),
    //f = f.replace("HEADER2.1", ""),
    
    //f = f.replace("HEADER2.2", d.split(',')[2]),
...
vizuly.theme.linearea = function (a) {
    ...
    } function d(b, c, d) { a.selection().selectAll(".vz-radial_progress-label").style("font-weight", 700) } function e(b, c, d) {
        a.selection().selectAll(".vz-radial_progress-label").style("font-weight", null)
    } function f() { i.forEach(function (b) { a.on(b.on, b.callback) }) } function g() {
        i.forEach(function (b) { a.on(b.on, null) })
    } var h = {
        Alert: {
            name: "Alert", label_color: "#CCC", track_fill: "#DDDDDD", progress_colors: ["#4CAF50", "#FFC107", "#FF9800", "#E64A19", "#FFEB3B"], arc_fill: function (a, b) {
                return this.progress_colors[b % 5]
            }, arc_fill_opacity: function (a, b) { return 1 }, arc_stroke: function (a, b) {
                return this.progress_colors[b % 5]
            }, "class": "vz-skin-alert"
        }, Fire: {
            name: "Fire", label_color: "#F13870", track_fill: "#DDDDDD", progress_colors: ["#C50A0A", "#F57C00", "#FF9800", "#FFEB3B", "#C2185B"],
