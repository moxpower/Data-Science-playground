...
function loadData() {
    ...
    viz = vizuly.viz.halo_cluster(document.getElementById("viz_container")), viz.data(data).width(800).height(600).haloKey(function (a) {
    ...
    theme.skin("custom"),
    /* title */
    viz_title = viz.selection().select("svg").append("text").attr("class", "title").style("font-family", "Raleway").attr("x", viz.width() / 2).attr("y", 40).attr("text-anchor", "middle").style("font-weight", 300).text("Programming languages progress by project"),
    ...
    var d = {},
        ...
        var g = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect(),
            ...
            i = g.left,
            j = g.top + document.body.scrollTop; createDataTip(i + b.r, j + b.r + 25, congress, b.values[0].PROJ_NAME, "Total Received: " + f)
    ...
    var d = viz.selection().selectAll(".vz-halo-link-path.halo-key_" + b.data.key),
        ...
        e = "$" + d3.format(",.0f")(e);
        var f = d3.selectAll(".vz-halo-node-plot")[0][0].getBoundingClientRect(); createDataTip(b.x + f.left + f.width / 2, b.y + f.top + 100, "POLITICAL ACTION COMMITTEE", b.data.values[0].LANGUAGE, "Total Donated: " + e)
} function link_onMouseOver(a, b, c) {
    var d = viz.selection().selectAll(".vz-halo-node.node-key_" + viz.nodeKey()(b.data)), e = d.datum(), f = "$" + d3.format(",.0f")(viz.value()(b.data)), g = b.data.Month + "/" + b.data.Day + "/" + b.data.Year, h = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect(); createDataTip(e.x + h.left + e.r, e.y + e.r + 25 + h.top, g, b.data.PROJ_NAME, "Received: " + f); var i = viz.selection().selectAll(".vz-halo-arc.halo-key_" + viz.haloKey()(b.data)); e = i.datum(), createPacLabel(e.x, e.y, e.data.values[0].LANGUAGE)
} function createDataTip(a, b, c, d, e) {
    var f = datatip.replace("HEADER1", c);
    f = f.replace("HEADER2", d),
    f = f.replace("HEADER3", e),
    d3.select("body").append("div").attr("class", "vz-halo-label").style("position", "absolute").style("top", b + "px").style("left", a - 125 + "px").style("opacity", 0).html(f).transition().style("opacity", 1)
} function createPacLabel(a, b, c) {
    var d = viz.selection().selectAll(".vz-halo-arc-plot").append("g").attr("class", "vz-halo-label").style("pointer-events", "none").style("opacity", 0); d.append("text").style("font-size", "11px").style("fill", theme.skin().labelColor).style("fill-opacity", .75).attr("text-anchor", "middle").attr("x", a).attr("y", b).text(c); var e = d[0][0].getBoundingClientRect(); d.insert("rect", "text").style("shape-rendering", "auto").style("fill", theme.skin().labelFill).style("opacity", .45).attr("width", e.width + 12).attr("height", e.height + 12).attr("rx", 3).attr("ry", 3).attr("x", a - 5 - e.width / 2).attr("y", b - e.height - 3), d.transition().style("opacity", 1)
